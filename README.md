...
2. **異動清單讀取 (`git status --porcelain -u`)**：
   ...
   - 頂部即時統計已暫存與未暫存檔案數量。
3. **全庫檔案總覽與搜尋 (`git ls-files`)**：
   - 支援「全庫檔案」分頁，可隨時檢視與搜尋 Repo 內的所有已追蹤檔案（即使無未提交修改）。
4. **檔案差異檢視 (Diff Viewer) 與歷史 Commit 對比**：
   - 點擊任意檔案即可即時展開 Unified Diff 檢視面板。
   - 輕量 GitHub 風格渲染：新增行綠色高亮（`+`）、刪除行紅色高亮（`-`）、區塊標頭淡藍色。
   - 支援雙重異動（如 `MM`）切換檢視已暫存/未暫存差異。
   - **歷史版本檢視 (`git log` & `git show`)**：提供歷史 Commit 下拉選單，可點選任意歷史版本查看該次提交對檔案的具體修改內容。
5. **單鍵全部暫存 (`git add .`)**：一鍵將所有異動加入暫存區，並自動更新狀態、同步刷新 Diff 與操作回饋。
6. **提交變更 (`git commit -m`)**：
   - 支援快速鍵（`Ctrl + Enter`）直接送出。
   - **防呆安全機制**：當暫存區無檔案時自動禁用 Commit 按鈕並顯示引導提示，避免觸發 Git CLI 的 `no changes added to commit` 錯誤。
7. **自動刷新與驗證**：提交成功後自動刷新清單並呈現 **Working Tree Clean** 狀態。
